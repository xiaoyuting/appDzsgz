 ...
 *********************************************************************************
 *
 * 在使用 Kit 的过程中如果出现 bug 请及时以以下任意一种方式联系我，我会及时修复 bug
 *
...
{
    NetManagerShare.sessionManager = [AFHTTPSessionManager manager];
    
//    NetManagerShare.requestSerializer = HttpRequestSerializerJSON;
//    NetManagerShare.responseSerializer = HttpResponseSerializerJSON;

    /*! 设置请求超时时间，默认：30秒 */
    NetManagerShare.timeoutInterval = 30;
    /*! 打开状态栏的等待菊花 */
    ...
    // 配置自建证书的Https请求
    [self _setupSecurityPolicy];
}

/**
 配置自建证书的Https请求，只需要将CA证书文件放入根目录就行
 */
+ (void)_setupSecurityPolicy
{
    ...
    if (cerSet.count == 0)
    {
        ...
        NetManagerShare.sessionManager.securityPolicy = securityPolicy;
    }
    else
    {
        /*! 自定义的CA证书配置如下： */
 ...
 */
+ (URLSessionTask *)_requestWithType:(HttpRequestType)type
                             isNeedCache:(BOOL)isNeedCache
                               urlString:(NSString *)urlString
                              parameters:(id)parameters
                            successBlock:(ResponseSuccessBlock)successBlock
                            failureBlock:(ResponseFailBlock)failureBlock
                           progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    AFHTTPSessionManager *scc = NetManagerShare.sessionManager;
    AFHTTPResponseSerializer *scc2 = scc.responseSerializer;
    AFHTTPRequestSerializer *scc3 = scc.requestSerializer;
    NSTimeInterval timeoutInterval = NetManagerShare.timeoutInterval;
    
    NSString *isCache = isNeedCache ? @"开启":@"关闭";
    CGFloat allCacheSize = [NetManagerCache _getAllHttpCacheSize];
    
    if (NetManagerShare.isOpenLog)
    {
        NSLog(@"\n******************** 请求参数 ***************************");
        NSLog(@"\n请求头: %@\n超时时间设置：%.1f 秒【默认：30秒】\nAFHTTPResponseSerializer：%@【默认：AFJSONResponseSerializer】\nAFHTTPRequestSerializer：%@【默认：AFJSONRequestSerializer】\n请求方式: %@\n请求URL: %@\n请求param: %@\n是否启用缓存：%@【默认：开启】\n目前总缓存大小：%.6fM\n", NetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, timeoutInterval, scc2, scc3, requestType, URLString, parameters, isCache, allCacheSize);
        NSLog(@"\n********************************************************");
    }
    ...
    // 读取缓存
    id responseCacheData = [NetManagerCache _httpCacheWithUrlString:urlString parameters:parameters];
    ...
    if (NetManagerShare.isSetQueryStringSerialization)
    {
        [NetManagerShare.sessionManager.requestSerializer setQueryStringSerializationWithBlock:^NSString * _Nonnull(NSURLRequest * _Nonnull request, id  _Nonnull parameters, NSError * _Nullable __autoreleasing * _Nullable error) {
    ...
    {
        sessionTask = [NetManagerShare.sessionManager GET:URLString parameters:parameters  progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            // 对数据进行异步缓存
            [NetManagerCache _setHttpCache:responseObject urlString:urlString parameters:parameters];
            [[weakSelf tasks] removeObject:sessionTask];
    ...
    {
        sessionTask = [NetManagerShare.sessionManager POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
            if (NetManagerShare.isOpenLog)
            {
    ...
    {
        sessionTask = [NetManagerShare.sessionManager PUT:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
    ...
    {
        sessionTask = [NetManagerShare.sessionManager DELETE:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            if (successBlock)
            {
                successBlock(responseObject);
            }
 ...
 */
+ (URLSessionTask *)_request_GETWithEntity:(DataEntity *)entity
                                  successBlock:(ResponseSuccessBlock)successBlock
                                  failureBlock:(ResponseFailBlock)failureBlock
                                 progressBlock:(DownloadProgressBlock)progressBlock
{
    if (!entity || ![entity isKindOfClass:[DataEntity class]]) {
        return nil;
    }
    return [self _requestWithType:HttpRequestTypeGet isNeedCache:entity.isNeedCache urlString:entity.urlString
...
/**
 ...
 @return URLSessionTask
 */
+ (URLSessionTask *)_request_POSTWithEntity:(DataEntity *)entity
                                   successBlock:(ResponseSuccessBlock)successBlock
                                   failureBlock:(ResponseFailBlock)failureBlock
                                  progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    return [self _requestWithType:HttpRequestTypePost isNeedCache:entity.isNeedCache urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
/**
 ...
 @return URLSessionTask
 */
+ (URLSessionTask *)_request_PUTWithEntity:(DataEntity *)entity
                                  successBlock:(ResponseSuccessBlock)successBlock
                                  failureBlock:(ResponseFailBlock)failureBlock
                                 progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    return [self _requestWithType:HttpRequestTypePut isNeedCache:NO urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
/**
 ...
 @return URLSessionTask
 */
+ (URLSessionTask *)_request_DELETEWithEntity:(DataEntity *)entity
                                     successBlock:(ResponseSuccessBlock)successBlock
                                     failureBlock:(ResponseFailBlock)failureBlock
                                    progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    return [self _requestWithType:HttpRequestTypeDelete isNeedCache:NO urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
/**
 ...
 @return URLSessionTask
 */
+ (URLSessionTask *)_uploadImageWithEntity:(DataEntity *)entity
                                  successBlock:(ResponseSuccessBlock)successBlock
                                   failurBlock:(ResponseFailBlock)failureBlock
                                 progressBlock:(UploadProgressBlock)progressBlock
{
    if (!entity || entity.urlString == nil || ![entity isKindOfClass:[ImageDataEntity class]]) {
        return nil;
    }
    
    ImageDataEntity *imageEntity = (ImageDataEntity *)entity;
    ...
    {
        NSLog(@"******************** 请求参数 ***************************");
        NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",NetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, @"POST",URLString, imageEntity.parameters);
        NSLog(@"********************************************************");
    }
    URLSessionTask *sessionTask = nil;
    sessionTask = [NetManagerShare.sessionManager POST:URLString parameters:imageEntity.parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        /*! 出于性能考虑,将上传图片进行压缩 */
        [imageEntity.imageArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([obj isKindOfClass:[PHAsset class]]) {
                ...
                [imageManager requestImageForAsset:asset targetSize:CGSizeMake(asset.pixelWidth , asset.pixelHeight) contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                    ...
                    [self _uploadImageWithFormData:formData resizedImage:result imageType:imageEntity.imageType imageScale:imageEntity.imageScale fileNames:imageEntity.fileNames index:idx];
                }];
            } else {
                /*! 此处是使用其他第三方相册，可以自由定制压缩方法 */
                resizedImage = obj;
                [self _uploadImageWithFormData:formData resizedImage:resizedImage imageType:imageEntity.imageType imageScale:imageEntity.imageScale fileNames:imageEntity.fileNames index:idx];
            }
        }];
 ...
 */
+ (void)_uploadVideoWithEntity:(DataEntity *)entity
                    successBlock:(ResponseSuccessBlock)successBlock
                    failureBlock:(ResponseFailBlock)failureBlock
                   progressBlock:(UploadProgressBlock)progressBlock
{
    if (!entity || entity.urlString == nil || ![entity isKindOfClass:[FileDataEntity class]]) {
        return;
    }
    FileDataEntity *fileEntity = (FileDataEntity *)entity;
    /*! 获得视频资源 */
    ...
    [avAssetExport exportAsynchronouslyWithCompletionHandler:^{
        switch ([avAssetExport status]) {
            ...
            {
                [NetManagerShare.sessionManager POST:fileEntity.urlString parameters:fileEntity.parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (URLSessionTask *)_downLoadFileWithEntity:(DataEntity *)entity
                                   successBlock:(ResponseSuccessBlock)successBlock
                                   failureBlock:(ResponseFailBlock)failureBlock
                                  progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    {
        NSLog(@"******************** 请求参数 ***************************");
        NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",NetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, @"download", fileEntity.urlString, fileEntity.parameters);
        NSLog(@"******************************************************");
    }
    
    URLSessionTask *sessionTask = nil;
    
    sessionTask = [NetManagerShare.sessionManager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        if (NetManagerShare.isOpenLog)
        {
...
+ (URLSessionTask *)_uploadFileWithWithEntity:(DataEntity *)entity
                                     successBlock:(ResponseSuccessBlock)successBlock
                                     failureBlock:(ResponseFailBlock)failureBlock
                                    progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    {
        NSLog(@"******************** 请求参数 ***************************");
        NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",NetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, @"uploadFile", fileEntity.urlString, fileEntity.parameters);
        NSLog(@"******************************************************");
    }
    URLSessionTask *sessionTask = nil;
    sessionTask = [NetManagerShare.sessionManager POST:fileEntity.urlString parameters:fileEntity.parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
 ...
 *  开启网络监测
 */
+ (void)_startNetWorkMonitoringWithBlock:(NetworkStatusBlock)networkStatus
{
    /*! 1.获得网络监控的管理者 */
 ...
 *  取消指定 URL 的 Http 请求
 */
+ (void)_cancelRequestWithURL:(NSString *)URL
{
    if (!URL)
    {
        return;
...
- (void)setTimeoutInterval:(NSTimeInterval)timeoutInterval
{
    ...
    NetManagerShare.sessionManager.requestSerializer.timeoutInterval = timeoutInterval;
}
...
- (void)setRequestSerializer:(HttpRequestSerializer)requestSerializer
{
    ...
    switch (requestSerializer) {
        ...
        {
            NetManagerShare.sessionManager.requestSerializer = [AFJSONRequestSerializer serializer] ;
        }
            break;
        case HttpRequestSerializerHTTP:
        {
            NetManagerShare.sessionManager.requestSerializer = [AFHTTPRequestSerializer serializer] ;
        }
            break;
...
- (void)setResponseSerializer:(HttpResponseSerializer)responseSerializer
{
    ...
    switch (responseSerializer) {
        ...
        {
            NetManagerShare.sessionManager.responseSerializer = [AFJSONResponseSerializer serializer] ;
        }
            break;
        case HttpResponseSerializerHTTP:
        {
            NetManagerShare.sessionManager.responseSerializer = [AFHTTPResponseSerializer serializer] ;
        }
            break;
...
- (void)setHttpHeaderFieldDictionary:(NSDictionary *)httpHeaderFieldDictionary
{
    ...
    for (NSInteger i = 0; i < keyArray.count; i ++)
    {
        ...
        [NetManager _setValue:valueString forHTTPHeaderKey:keyString];
    }
}
 ...
 *  自定义请求头
 */
+ (void)_setValue:(NSString *)value forHTTPHeaderKey:(NSString *)HTTPHeaderKey
{
...
{
    [NetManagerShare.sessionManager.requestSerializer clearAuthorizationHeader];
}

+ (void)_uploadImageWithFormData:(id<AFMultipartFormData>  _Nonnull )formData
...
{
    [NetManagerCache _clearAllHttpCache];
}
...
#ifdef DEBUG
@implementation NSArray (NetManager)
...
@implementation NSDictionary (NetManager)
